...
{
    public sealed class RgbaWeightAnalyzer : WeightAnalyzer<Rgba32>
    {
        protected override PointF GetWeight(Image<Rgba32> weightMap, Rgba32 backgroundColor)
        {
            ...
            weightMap.ProcessPixelRows((accessor) =>
            {
                for (var y = 0; y < h; y++)
                {
                    ...
                    for (var x = 0; x < w; x++)
                    {
                        ...
                        var d = (0.299 * rd + 0.587 * gd + 0.114 * bd) * c.A * Constants.BytePrecision;
